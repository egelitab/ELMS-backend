...
async function run() {
  try {
    await pool.query(
      ...
      BEGIN
          ...
          IF NOT FOUND THEN
              ...
              VALUES ('Abebe', 'Kebede', 'abebe_internet_prog@bdu.edu.et', 'hash', 'instructor', 'Dr.')
          ...
          SELECT id INTO dummy_student_id FROM users WHERE email = 'dummy_student_is_a@bdu.edu.et';
          IF NOT FOUND THEN
              INSERT INTO users (first_name, last_name, email, password_hash, role, department_id, section)
              VALUES ('Dummy', 'Student', 'dummy_student_is_a@bdu.edu.et', 'hash', 'student', info_sys_id, 'Section A')
    ...
    console.log('Database relationships synced perfectly!');
  } catch (e) {
    console.error(e);
  } finally {
    process.exit(0);
